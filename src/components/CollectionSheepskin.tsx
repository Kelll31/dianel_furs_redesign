...
import { ArrowRight, Sun, Wind, Heart, Shield, ShoppingBag, Scale } from "lucide-react";
...
const sheepskinFeatures = [
  {
    icon: <Sun size={24} />,
    title: "Тепло",
    description: "Натуральная овчина меринос сохраняет тепло даже в сильные морозы"
  },
  {
    icon: <Wind size={24} />,
    title: "Воздухопроницаемость",
    description: "Естественная терморегуляция для комфорта в любую погоду"
  },
  {
    icon: <Heart size={24} />,
    title: "Мягкость",
    description: "Невероятно нежная текстура, приятная к телу"
  },
  {
    icon: <Shield size={24} />,
    title: "Защита",
    description: "Природный ланолин защищает от влаги и холода"
  }
];

export default function CollectionSheepskin() {
  ...
  // Фильтруем товары категории "Овчина"
  const sheepskinProducts = products.filter(p => p.category === "Овчина");

  return (
    <div ref={containerRef} className="bg-bg min-h-screen relative">
      <Helmet>
        <title>Овчина | Dianel Furs - Уютное тепло</title>
        <meta name="description" content="Коллекция из овчины Dianel Furs. Натуральное тепло, мягкость и комфорт премиум-класса." />
        <meta name="keywords" content="овчина, меринос, дубленки, пальто из овчины, натуральная овчина" />
      </Helmet>
        ...
        <div className="relative z-10 text-center px-6">
          ...
          >
            ...
            <h1 className="text-5xl sm:text-6xl md:text-7xl lg:text-[6.5vw] font-serif leading-[0.9] tracking-tighter mb-12">
              Овчина <br /> <span className="italic text-accent">Уют</span>
            </h1>
            <div className="flex flex-col md:flex-row justify-center items-center gap-8 text-[10px] uppercase tracking-[0.4em] text-muted">
              <span className="flex items-center gap-2"><Sun size={12} className="text-accent" /> Natural Warmth</span>
              <span className="hidden md:block text-accent/20">|</span>
              <span className="flex items-center gap-2"><Heart size={12} className="text-accent" /> Incredibly Soft</span>
              <span className="hidden md:block text-accent/20">|</span>
              <span className="flex items-center gap-2"><Wind size={12} className="text-accent" /> Breathable</span>
            </div>
          </motion.div>
        </div>
      ...
      <section className="py-32 px-6 md:px-12 relative bg-white/5">
        <div className="max-w-7xl mx-auto">
          <div className="text-center mb-20">
            ...
            >
              Природное <span className="text-accent">Совершенство</span>
            </motion.h2>
          </div>

          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
            {sheepskinFeatures.map((feature, idx) => (
              <motion.div
                key={idx}
      ...
      <section className="py-32 px-6 md:px-12">
        <div className="max-w-7xl mx-auto">
          <div className="mb-20">
            ...
            >
              Изделия из <span className="italic text-accent">овчины</span>
            </motion.h2>
          </div>

          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-12">
            {sheepskinProducts.map((product, idx) => (
              <motion.div
                key={product.id}
                initial={{ opacity: 0, y: 50 }}
                whileInView={{ opacity: 1, y: 0 }}
      ...
      <section className="py-32 px-6 md:px-12 relative overflow-hidden">
        ...
        <div className="max-w-4xl mx-auto text-center relative z-10">
          ...
          >
            Тепло <span className="italic text-accent">и нежность</span>
          </motion.h2>
          <motion.p
            initial={{ opacity: 0, y: 30 }}
            whileInView={{ opacity: 1, y: 0 }}
          ...
          >
            Овчина меринос — это естественное тепло, невероятная мягкость 
            и уют в каждом прикосновении. Классика вне времени.
          </motion.p>
          <motion.div
            initial={{ opacity: 0, y: 30 }}
            whileInView={{ opacity: 1, y: 0 }}
