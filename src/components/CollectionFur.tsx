...
import { ArrowRight, Crown, Gem, Star, Sparkles, ShoppingBag, Scale, Heart } from "lucide-react";
...
const furFeatures = [
  {
    icon: <Crown size={24} />,
    title: "Элитность",
    description: "Норка, соболь, лиса — вершина мехового искусства для самых взыскательных"
  },
  {
    icon: <Gem size={24} />,
    title: "Эксклюзив",
    description: "Аукционный мех премиум-класса с уникальными характеристиками"
  },
  {
    icon: <Star size={24} />,
    title: "Роскошь",
    description: "Неповторимая природная красота и благородный блеск"
  },
  {
    icon: <Sparkles size={24} />,
    title: "Мастерство",
    description: "Ручная работа и внимание к каждой детали изделия"
  }
];

export default function CollectionFur() {
  ...
  // Фильтруем товары категории "Пушнина"
  const furProducts = products.filter(p => p.category === "Пушнина");

  return (
    <div ref={containerRef} className="bg-bg min-h-screen relative">
      <Helmet>
        <title>Пушнина | Dianel Furs - Королевский выбор</title>
        <meta name="description" content="Коллекция из пушнины Dianel Furs. Норка, соболь, лиса — элитный мех премиум-класса." />
        <meta name="keywords" content="пушнина, норка, соболь, лиса, песец, элитный мех, шубы" />
      </Helmet>
      ...
      <section className="relative h-screen flex items-center justify-center overflow-hidden">
        ...
        >
          <img
            src="https://images.unsplash.com/photo-1539109132314-34a9c655a8c0?q=80&w=2000&auto=format&fit=crop"
            alt="Fur Hero"
        ...
        <div className="relative z-10 text-center px-6">
          ...
          >
            ...
            <h1 className="text-5xl sm:text-6xl md:text-7xl lg:text-[6.5vw] font-serif leading-[0.9] tracking-tighter mb-12">
              Пушнина <br /> <span className="italic text-accent">Величие</span>
            </h1>
            <div className="flex flex-col md:flex-row justify-center items-center gap-8 text-[10px] uppercase tracking-[0.4em] text-muted">
              <span className="flex items-center gap-2"><Crown size={12} className="text-accent" /> Elite Grade</span>
              <span className="hidden md:block text-accent/20">|</span>
              <span className="flex items-center gap-2"><Gem size={12} className="text-accent" /> Exclusive</span>
              <span className="hidden md:block text-accent/20">|</span>
              <span className="flex items-center gap-2"><Star size={12} className="text-accent" /> Luxury</span>
            </div>
          </motion.div>
        </div>
      ...
      <section className="py-32 px-6 md:px-12 relative bg-white/5">
        <div className="max-w-7xl mx-auto">
          <div className="text-center mb-20">
            ...
            >
              Искусство <span className="text-accent">Роскоши</span>
            </motion.h2>
          </div>

          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
            {furFeatures.map((feature, idx) => (
              <motion.div
                key={idx}
      ...
      <section className="py-32 px-6 md:px-12">
        <div className="max-w-7xl mx-auto">
          <div className="mb-20">
            ...
            >
              Изделия из <span className="italic text-accent">пушнины</span>
            </motion.h2>
          </div>

          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-12">
            {furProducts.map((product, idx) => (
              <motion.div
                key={product.id}
                initial={{ opacity: 0, y: 50 }}
                whileInView={{ opacity: 1, y: 0 }}
      ...
      <section className="py-32 px-6 md:px-12 relative overflow-hidden">
        ...
        <div className="max-w-4xl mx-auto text-center relative z-10">
          ...
          >
            Роскошь <span className="italic text-accent">без границ</span>
          </motion.h2>
          <motion.p
            initial={{ opacity: 0, y: 30 }}
            whileInView={{ opacity: 1, y: 0 }}
          ...
          >
            Пушнина — это выбор тех, кто не идёт на компромиссы. 
            Норка, соболь, лиса — символы статуса и безупречного вкуса.
          </motion.p>
          <motion.div
            initial={{ opacity: 0, y: 30 }}
            whileInView={{ opacity: 1, y: 0 }}
