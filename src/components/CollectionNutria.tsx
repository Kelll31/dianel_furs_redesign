...
import { ArrowRight, ShieldCheck, Droplets, Clock, Award, ShoppingBag, Scale, Heart } from "lucide-react";
...
const nutriaFeatures = [
  {
    icon: <Droplets size={24} />,
    title: "Влагостойкость",
    description: "Природная защита от влаги и снега. Идеально для переменчивой погоды"
  },
  {
    icon: <ShieldCheck size={24} />,
    title: "Износостойкость",
    description: "Высокая прочность и долговечность. Служит многие сезоны"
  },
  {
    icon: <Clock size={24} />,
    title: "Практичность",
    description: "Неприхотлив в уходе. Сохраняет презентабельный вид годами"
  },
  {
    icon: <Award size={24} />,
    title: "Качество",
    description: "Отборная стриженой нутрия премиум-класса"
  }
];

export default function CollectionNutria() {
  ...
  // Фильтруем товары категории "Нутрия"
  const nutriaProducts = products.filter(p => p.category === "Нутрия");

  return (
    <div ref={containerRef} className="bg-bg min-h-screen relative">
      <Helmet>
        <title>Нутрия | Dianel Furs - Практичная элегантность</title>
        <meta name="description" content="Коллекция из нутрии Dianel Furs. Практичность, износостойкость и элегантность в каждой детали." />
        <meta name="keywords" content="нутрия, шубы из нутрии, стриженой нутрия, мех нутрии" />
      </Helmet>
        ...
        <div className="relative z-10 text-center px-6">
          ...
          >
            ...
            <h1 className="text-5xl sm:text-6xl md:text-7xl lg:text-[6.5vw] font-serif leading-[0.9] tracking-tighter mb-12">
              Нутрия <br /> <span className="italic text-accent">Классика</span>
            </h1>
            <div className="flex flex-col md:flex-row justify-center items-center gap-8 text-[10px] uppercase tracking-[0.4em] text-muted">
              <span className="flex items-center gap-2"><Droplets size={12} className="text-accent" /> Water Resistant</span>
              <span className="hidden md:block text-accent/20">|</span>
              <span className="flex items-center gap-2"><ShieldCheck size={12} className="text-accent" /> Durable</span>
              <span className="hidden md:block text-accent/20">|</span>
              <span className="flex items-center gap-2"><Clock size={12} className="text-accent" /> Timeless</span>
            </div>
          </motion.div>
        </div>
      ...
      <section className="py-32 px-6 md:px-12 relative bg-white/5">
        <div className="max-w-7xl mx-auto">
          <div className="text-center mb-20">
            ...
            >
              Сила <span className="text-accent">и</span> Стиль
            </motion.h2>
          </div>

          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
            {nutriaFeatures.map((feature, idx) => (
              <motion.div
                key={idx}
      ...
      <section className="py-32 px-6 md:px-12">
        <div className="max-w-7xl mx-auto">
          <div className="mb-20">
            ...
            >
              Изделия из <span className="italic text-accent">нутрии</span>
            </motion.h2>
          </div>

          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-12">
            {nutriaProducts.map((product, idx) => (
              <motion.div
                key={product.id}
                initial={{ opacity: 0, y: 50 }}
                whileInView={{ opacity: 1, y: 0 }}
      ...
      <section className="py-32 px-6 md:px-12 relative overflow-hidden">
        <div className="absolute inset-0 z-0">
          <img
            src="https://images.unsplash.com/photo-1551488831-00ddcb6c6bd3?q=80&w=2000&auto=format&fit=crop"
            alt="CTA Background"
        ...
        <div className="max-w-4xl mx-auto text-center relative z-10">
          ...
          >
            Надёжность <span className="italic text-accent">в каждой детали</span>
          </motion.h2>
          <motion.p
            initial={{ opacity: 0, y: 30 }}
            whileInView={{ opacity: 1, y: 0 }}
          ...
          >
            Нутрия — выбор тех, кто ценит практичность без компромиссов. 
            Износостойкость, защита от влаги и timeless-дизайн.
          </motion.p>
          <motion.div
            initial={{ opacity: 0, y: 30 }}
            whileInView={{ opacity: 1, y: 0 }}
